...
from prompts import FINAL_ANSWER, INIT_PROMPT
...
class SearchService:
    ...
    def query_chunks(self, txt: str, document_id: str, qdrant_service) -> list[dict]:
        ...
        chunks = [
            {"text": chunk.get("text", "")[:1200], "score": chunk.get("score", "")}
            for chunk in chunks
        ]
        return chunks

    def query_question(
        self, txt: str, chunks: list[dict], prompt: str, llm_service, params=None
    ) -> dict:
    ...
    def build_planner_messages(
        self, user_query: str, chunks: list[dict], prompt: str
    ) -> list[dict]:
        return [
            {
                "role": "system",
            ...
            },
            {
                "role": "user",
                "content": (
                    f"Contexto: {chunks}" f"Pergunta do usuário:\n{user_query}"
                ),
            },
        ]

    def search(
        self, user_query: str, document_id: str, llm_service, qdrant_service, logger
    ):
        chunks_list = []
        i = 0
        ...
        while i < limit:
            ...
            chunks_list.extend(
                self.query_chunks(user_query, document_id, qdrant_service)
            )
            response = self.query_question(
                user_query, chunks_list, INIT_PROMPT, llm_service
            )
        ...
        return self.query_question(
            initial_question, chunks_list, FINAL_ANSWER, llm_service, params=params
        )
